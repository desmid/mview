...
1. Save the archive to your software area, e.g., ``/usr/local``, then uncompress
   and extract it::
     ...
     tar xvzf mview-1.61.tar.gz

   or::

     gunzip < mview-1.61.tar.gz | tar xvf -

   This would create a directory called ``mview-1.61`` and place all the files
   under there.
...
4. Find the ``use lib '/path/to/mview/lib';`` line and change it, in our
   example, to::
     ...
     use lib '/usr/local/mview-1.61/lib';
