...
$Bio::Parse::Format::FT::indent  = 16;

#skip these many chars at front of each raw record
$Bio::Parse::Format::FT::skip    =  5;
...
sub new {
	...
	if ($line =~ /^(\S+)/) {
		...
		$text->scan_nest($Bio::Parse::Format::FT::indent, $1);
		next;
	    }
	}
	...
	$line =~ tr/ //d;
	$text->scan_nest($Bio::Parse::Format::FT::indent, "$line");
        $self->warn("unknown field: $line");
    }
    $self;
}

#Override Bio::Parse::Record::get_class() which returns the Record hierarchy
#of $self (which could involve Embl or Genbank, etc), to return the generic
#feature table class defined here. Used by Bio::Parse::Record::add_object().
sub get_class {
    'Bio::Parse::Format::FT';
}
...
sub _parse_location {
    ...
    #cross-reference
    if ($loc =~ /^($Bio::Parse::Format::FT::ac_accession):(.*)$/o) {
	$op = $1;
...
###########################################################################
package Bio::Parse::Format::FT::CDS;

use vars qw(@ISA);

@ISA = qw(Bio::Parse::Feature);

sub new {
    new Bio::Parse::Feature(@_);
}


###########################################################################
package Bio::Parse::Format::FT::tRNA;
...
###########################################################################
package Bio::Parse::Format::FT::rRNA;
...
###########################################################################
package Bio::Parse::Format::FT::exon;
