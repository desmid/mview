...
sub readable {
    if (@_ < 1) {
	Bio::Parse::Message::die('Bio::Parse::Index',
...
sub new {
    ...
    warn "Bio::Parse::Index opened dbm  file ($self->{'class'}, $self->{'dbm'}, '$self->{'mode'}')\n"
	if $VERBOSE;
    $self;
}
...
sub DESTROY {
	...
	if (defined $self->{'data_fh'}->{$fh}) {
	    ...
	    warn "Bio::Parse::Index closed data file ($fh, 'r')\n"
		if $VERBOSE;
	}
    }
}
...
sub get_entry {
    ...
    $entry = Bio::Parse::Entry->new($self->{'class'});

#    $entry = "new Bio::Parse::Entry(\$self->{'class'})";
#    $entry = eval $entry;
#    if ($@) {
#	chomp $@;
...
sub _extract_entry {
	...
	warn "Bio::Parse::Index opened data file ($file, 'r')\n" if $VERBOSE;
